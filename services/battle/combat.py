...
class CombatSystem:
    ...
    def _get_value(obj, key, default=0):
        """Универсальный метод для получения значения из объекта или словаря"""
        if hasattr(obj, key):
    ...
    def calculate_damage(attacker, defender):
        ...
        attacker_strength = CombatSystem._get_value(attacker, 'strength', 0)
        attacker_level = CombatSystem._get_value(attacker, 'level', 1)
        defender_armor = CombatSystem._get_value(defender, 'armor', 0)
        defender_level = CombatSystem._get_value(defender, 'level', 1)
    ...
    async def fight(user_char, monster):
        ...
        monster_health = max(0, CombatSystem._get_value(monster, 'health') - user_damage)
    ...
    async def update_experience(session, user_id, exp_gained):
        ...
        char.health = min(100 + (new_level - 1) * 20, 1000)
